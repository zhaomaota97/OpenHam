...
OpenHam is a lightweight desktop AI companion that appears instantly when you need it — no window switching, no browser tabs, just press `Ctrl+F11` and start typing. Built for creators, developers, and anyone who wants AI assistance without the friction.
...
|----------------|---------|
| Open browser → Find tab → Click input | Press `Ctrl+F11` |
...
3. Run `OpenHam.exe`
4. Press `Ctrl+F11` to activate

### Core Features

✅ **Global Hotkey Activation** — Press `Ctrl+F11` anywhere, anytime (fully customizable)  
✅ **Streaming AI Responses** — Real-time token-by-token output, no waiting  
✅ **Instant Math Calculation** — Type expressions like `sqrt(144) + 5^2`, get immediate results  
...
|-----------|-------------|---------|
| `hotkey` | Global hotkey (pynput format) | `<ctrl>+<F11>` |
...
2. **Trigger window**: Press the configured hotkey (default `Ctrl+F11`)
3. **Enter query**:
   - Plain text: Send to AI for streaming response
   - Math expression (e.g., `2+2*3`): Real-time calculation display
4. **Press Esc**: Close the window and return to your work

### Technical Stack

- **UI Framework**: PyQt6 — Native, lightweight, cross-platform
- **Hotkey Monitoring**: pynput — Global keyboard listener
...
OpenHam 是一个轻量级桌面 AI 伴侣，需要时即刻出现——无需切换窗口、无需打开浏览器标签，只需按下 `Ctrl+F11` 就能开始输入。专为创作者、开发者以及所有希望无摩擦使用 AI 的人打造。
...
✅ **全局热键激活** — 随时随地按 `Ctrl+F11`（完全可自定义）  
✅ **流式 AI 响应** — 实时逐字输出，无需等待  
✅ **即时数学计算** — 输入表达式如 `sqrt(144) + 5^2`，立即获得结果  
✅ **系统托盘集成** — 后台运行，零任务栏占用  
✅ **单实例保护** — 防止多个实例冲突  
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `hotkey` | 全局热键（pynput 格式） | `<ctrl>+<F11>` |
| `model` | 使用的 AI 模型 | `deepseek-chat` |
| `temperature` | 响应创造性（0-1） | `0.7` |
